...
const ASSIGN_TPL = {
    mini_project: (t, s, code, dur) => `<strong>🔧 Mini Project — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Build a <strong>working project</strong> that solves a real problem using Google research and important Anna University concepts from <em>${t}</em>.<br><br>` +
        `<strong>Deliverables:</strong><ul>` +
        `<li>Problem statement &amp; scope</li>` +
        `<li>Working implementation (source code + executable/demo)</li>` +
        `<li>Project report &amp; live ${dur} demo</li></ul>`,

    case_study: (t, s, code, dur) => `<strong>🔍 Case Study — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Analyze a comprehensive case study related to <strong>${t}</strong> based on Google research and important Anna University case study topics.<br><br>` +
        `<strong>Report structure:</strong><ul>` +
        `<li>Problem / challenge identified</li>` +
        `<li>Solution approach &amp; technologies used</li>` +
        `<li>Lessons learned &amp; live ${dur} presentation</li></ul>` +
        `Cite credible open sources.`,

    literature_review: (t, s, code, dur) => `<strong>📚 Literature Review — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Survey recent research on <strong>${t}</strong> within the domain of <em>${s}</em>.<br><br>` +
        `<strong>Requirements:</strong><ul>` +
        `<li>Review relevant papers (IEEE, ACM, Springer)</li>` +
        `<li>Identify 2–3 open research gaps</li>` +
        `<li>Live ${dur} explanation to reviewers</li></ul>`,

    practical: (t, s, code, dur) => `<strong>⚗️ Practical Exercise — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Conduct a hands-on experiment strictly based on the official Anna University syllabus for <strong>${t}</strong>.<br><br>` +
        ...
        `Duration: ${dur}. Viva questions will test underlying syllabus theory.`,

    problem_solve: (t, s, code, dur) => `<strong>🧮 Problem Set — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Solve curated numericals, derivations, or algorithmic challenges on <strong>${t}</strong> drawn exactly from Anna University repeated past papers.<br><br>` +
        `<strong>Submission rules:</strong><ul>` +
        `<li>Show complete working — no step skipped</li>` +
        `<li>Include 1 self-composed problem with full solution</li>` +
        `<li>Live ${dur} step-by-step walkthrough to reviewers</li></ul>`,

    presentation: (t, s, code, dur) => `<strong>🎤 Topic Presentation — ${t}</strong><br><em>${code}: ${s}</em><br><br>` +
        `Prepare and deliver a <strong>${dur} presentation</strong> on <strong>${t}</strong>. Topics must cover Anna University important questions and past year questions.<br><br>` +
        `<strong>Slide deck (10–12 slides):</strong><ul>` +
        `<li>Core concepts, diagrams &amp; worked examples</li>` +
        `<li>Real-world applications &amp; recent advances</li>` +
        `<li>Interactive Q&amp;A</li></ul>`
};
...
function buildPhase3Description(aType, topic, code, subject, bloomLevel, dur) {
    ...
    const baseTpl = (ASSIGN_TPL[aType] || ASSIGN_TPL.presentation)(topic, subject, code, dur);
...
function logicGeneratePhase3Assignments(teams, unitData, cycleNum, config = {}) {
    ...
    const { courseName = '', courseCode = '', assignType = 'mixed', duration = '10-15 min', focusUnits = [1, 2, 3, 4, 5] } = config;
    ...
    const activeTypes = assignType === 'mixed' ? ASGN_TYPE_ORDER : [assignType];
    
    // Flatten topics from unitData with unit filtering
    let topicPool = [];
    unitData.forEach(u => {
        if (!focusUnits.includes(parseInt(u.id))) return;
        const topics = (u.topics || []).length > 0 ? u.topics : [u.title];
        topics.forEach(t => topicPool.push({ title: t, unitId: u.id, unitTitle: u.title }));
    });
    // Fallback if no topics exist in the ticked units
    if (!topicPool.length) topicPool = [{ title: "General Context", unitId: 1, unitTitle: "Default Unit" }];
    ...
    while (assignments.length < totalNeeded) {
        ...
        const aType = activeTypes[tIdx % activeTypes.length];
        ...
        if (!usedCombos.has(combo)) {
            ...
            assignments.push({
                ...
                description: buildPhase3Description(aType, topic.title, courseCode, courseName, bloom, duration),
